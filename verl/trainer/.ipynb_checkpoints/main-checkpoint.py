...
class Runner:
    ...
    def run(self, config: PPOConfig):
        ...
        # Configure Ray resource allocation for reward workers
        ray_options = {"num_cpus": config.worker.reward.num_cpus}
        if config.worker.reward.num_gpus > 0:
            ray_options["num_gpus"] = config.worker.reward.num_gpus
            print(f"🔧 Reward workers will use {config.worker.reward.num_gpus} GPUs each")
        
        RemoteRewardManager = ray.remote(RewardManager).options(**ray_options)
