import os, re, json, time, requests, concurrent.futures, regex
from collections import Counter, defaultdict
from typing import Any, Dict, List, Optional
from pathlib import Path

import numpy as np
from tqdm import tqdm

def gpt_4_call(text: str, api_key: str, url: str,
               model: str = "gpt-4o-mini",
               retries: int = 5, delay: int = 25) -> str:
    payload = json.dumps({
        "model": model,
        "messages": [{"role": "user", "content": text}]
    })
    headers = {
        "Accept": "application/json",
        "Authorization": api_key, 
        "Content-Type": "application/json"
    }
    last_err = None
    for i in range(retries):
        try:
            r = requests.post(url, headers=headers, data=payload, timeout=20)
            r.raise_for_status()
            return r.json()["choices"][0]["message"]["content"]
        except Exception as e:
            last_err = e
            print(f"[Retry {i+1}/{retries}] {e}")
            time.sleep(delay)
    return f"__API_ERROR__: {str(last_err)}"

json_obj_pat   = regex.compile(r"\{(?:[^{}]|(?R))*\}")
json_array_pat = regex.compile(r"\[(?:[^\[\]]|(?R))*\]")

def extract_scores(raw: str):
    txt = (raw or "").strip()
    m_arr = json_array_pat.search(txt)
    m_obj = json_obj_pat.search(txt)
    try:
        objs = json.loads(m_arr.group() if m_arr else m_obj.group())
        if isinstance(objs, dict):
            objs = [objs]
    except Exception:
        return []
    res = []
    for o in objs:
        res.append([
            int(o.get("Rarity", 0)),
            int(o.get("Complexity", 0)),
            int(o.get("Informativeness", 0)),
            int(o.get("Overall rating", 0) or o.get("Overall", 0) or 0),
        ])
    return res


def compress_overall(vals):
    print("Original distribution :", Counter(vals))
    comp = [min(9, max(4, int(v))) - 4 for v in vals] 
    print("Compressed distribution:", Counter(comp))
    return comp

TASK_TYPES = ("qa", "mcq", "para", "tfq", "cs")

def _norm(s: Optional[str]) -> str:
    return (s or "").strip().lower()

def infer_task_type(rec: Dict[str, Any]) -> str:
    src = _norm(rec.get("source"))
    if src in TASK_TYPES:
        return src

    rid = _norm(str(rec.get("id", "")))
    if rid:
        tokens = re.split(r"[\s_\-:/\.]+", rid)
        for t in TASK_TYPES:
            if t in tokens:
                return t
        for t in TASK_TYPES:
            if re.search(fr"(?:^|[_\-\s]){t}(?:$|[_\-\s])", rid):
                return t
    return "qa"

def get_generated_output(rec: Dict[str, Any]) -> str:
    txt = rec.get("generated_output")
    if isinstance(txt, str) and txt.strip():
        return txt.strip()
    txt = rec.get("output")
    if isinstance(txt, str) and txt.strip():
        return txt.strip()
    return ""

def build_rubric(task: str) -> str:
    base = (
        "You are a data-quality estimator. "
        "Rate this {task_desc} on "
        "Rarity, Complexity, Informativeness, and Overall (1-10 integers).\n"
        "Return ONLY a JSON object like:\n"
        '{{ "Rarity": <int>, "Complexity": <int>, '
        '"Informativeness": <int>, "Overall rating": <int> }}\n\n'
        "### {task_name}:\n"
    )

    task_map = {
        "qa":   ("**single** QA pair", "QA Pair"),
        "mcq":  ("**single** MCQ item", "MCQ Item"),
        "para": ("**single** merged paragraph", "Paragraph"),
        "tfq":  ("**single** True/False question", "TFQ"),
        "cs":   ("**single** category–statement example", "CS Example"),
    }

    task_desc, task_name = task_map.get(task, ("**single** QA pair", "QA Pair"))
    return base.format(task_desc=task_desc, task_name=task_name)

def merge_five_jsonl(input_dir: str, merged_out: Path) -> int:
    p = Path(input_dir)
    if not p.exists() or not p.is_dir():
        raise FileNotFoundError(f"Input dir not found or not a directory: {input_dir}")

    order = ["cs", "mcq", "para", "qa", "tfq"]
    patterns = {k: list(p.glob(f"*_{k}.jsonl")) for k in order}

    written = 0
    merged_out.parent.mkdir(parents=True, exist_ok=True)
    with merged_out.open("w", encoding="utf-8") as fw:
        for tag in order:
            files = patterns[tag]
            if not files:
                print(f"[WARN] No file matched '*_{tag}.jsonl' in {input_dir}, skip.")
                continue
            for fp in sorted(files):
                cnt = 0
                with fp.open("r", encoding="utf-8") as fr:
                    for line in fr:
                        line = line.strip()
                        if not line:
                            continue
                        fw.write(line + "\n")
                        written += 1
                        cnt += 1
                print(f"[INFO] Merged {cnt:>6} lines from {fp.name}")
    print(f"[INFO] Merged total lines: {written}")
    return written

def main(input_dir        : str = "infer_out/qwe_1_5b_grpo", 
         output_dir       : str = "rating",
         api_key          : str = "sk-xxx",
         url              : str = "https://api.openai.com/v1/chat/completions",
         model            : str = "gpt-4o-mini",
         max_workers      : int = 128):

    in_dir = Path(input_dir)
    if not in_dir.exists() or not in_dir.is_dir():
        raise FileNotFoundError(f"{in_dir} not found or not a directory")

    os.makedirs(output_dir, exist_ok=True) 
    merged_name = f"merged_{in_dir.name}.jsonl"
    merged_path = in_dir / merged_name     

    total_merged = merge_five_jsonl(str(in_dir), merged_path)
    if total_merged == 0:
        print("No data found to merge. Exit.")
        return

    records: List[Dict[str, Any]] = []
    with merged_path.open("r", encoding="utf-8") as fr:
        for line in fr:
            line = line.strip()
            if not line:
                continue
            try:
                records.append(json.loads(line))
            except Exception:
                pass

    if not records:
        print("No valid lines found after merge."); return

    prompts = []
    tasks   = []
    for rec in records:
        task = infer_task_type(rec)    
        tasks.append(task)
        rubric = build_rubric(task)
        out_text = get_generated_output(rec)  
        prompts.append(f"{rubric}\n\n### Output:\n{out_text}\n\n### Rating:")

    scores_per_rec = [[] for _ in records]

    def _worker(idx, prompt):
        return idx, gpt_4_call(prompt, api_key, url, model=model)

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
        futs = {pool.submit(_worker, i, p): i for i, p in enumerate(prompts)}
        for fut in tqdm(concurrent.futures.as_completed(futs),
                        total=len(futs), desc="Scoring"):
            i, raw = fut.result()
            scores = extract_scores(raw) or [[0, 0, 0, 0]]
            scores_per_rec[i] = scores[0]

    out_path = Path(output_dir) / f"scored_{merged_path.name}"

    overall_vals = [int(sc[3]) for sc in scores_per_rec]
    overall_compressed = compress_overall(overall_vals)

    per_type_raw   = defaultdict(list)   # 1–10
    per_type_comp  = defaultdict(list)   # 0–5

    with out_path.open("w", encoding="utf-8") as fw:
        for rec, sc, comp, task in zip(records, scores_per_rec, overall_compressed, tasks):
            rec["rating_detail"] = {
                "Rarity": sc[0],
                "Complexity": sc[1],
                "Informativeness": sc[2],
                "Overall": sc[3],
                "Overall_compressed": comp
            }
            rec["rated_task_type"] = task
            fw.write(json.dumps(rec, ensure_ascii=False) + "\n")
            per_type_raw[task].append(int(sc[3]))
            per_type_comp[task].append(int(comp))

    raw_mean = float(np.mean(overall_vals)) if overall_vals else 0.0
    cmp_mean = float(np.mean(overall_compressed)) if overall_compressed else 0.0

    summary_lines = [
        "==== SUMMARY ====",
        f"Merged from dir                : {in_dir}",
        f"Total items scored             : {len(overall_vals)}",
        f"Global Overall (raw, 1-10)     : {raw_mean:.2f}",
        f"Global Overall (compressed 0-5): {cmp_mean:.2f}",
        "",
        "---- Per-type Averages ----",
    ]
    for t in sorted(per_type_raw.keys()):
        r = per_type_raw[t]
        c = per_type_comp[t]
        if r and c:
            summary_lines.append(
                f"{t:>4} | n={len(r):4d} | raw={np.mean(r):.2f} | compressed={np.mean(c):.2f}"
            )
    summary_lines.append("==================")

    summary_txt = "\n".join(summary_lines)
    print(summary_txt)

    summary_path = Path(output_dir) / f"summary_{merged_path.stem}.txt"
    with summary_path.open("w", encoding="utf-8") as sf:
        sf.write(summary_txt)

    print(f"★ Merged JSONL  → {merged_path}")
    print(f"★ Scored JSONL  → {out_path}")
    print(f"★ Summary TXT   → {summary_path}")

if __name__ == "__main__":
    import fire
    fire.Fire(main)